...
int pcl_setattr(lua_State* state, USER* sd, char* attrname) {
  ...
  else if (!strcmp(attrname, "bonusMight")) sd->bonusMight = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusGrace")) sd->bonusGrace = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusWill")) sd->bonusWill = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusArmor")) sd->bonusArmor = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusDamage")) sd->bonusDamage = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusCrit")) sd->bonusCrit = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusHealing")) sd->bonusHealing = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusLifeSteal")) sd->bonusLifeSteal = lua_tonumber(state, -1);
  else if (!strcmp(attrname, "bonusReflect")) sd->bonusReflect = lua_tonumber(state, -1);
