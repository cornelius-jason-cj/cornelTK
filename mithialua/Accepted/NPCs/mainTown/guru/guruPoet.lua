PoetGuruNpc = {
    ...
    if choice == "Quest Armor" then
      ...
      choice2 = player:menuString(
			...
			if choice2 == "Yes" then
				if player:hasItem("poet_bone", 25) == true and
				player:hasItem('large_amber_ring', 1) == true then
					...
					player:removeItem("poet_bone",25)
					player:removeItem("large_amber_ring",1)

                    player.registry["third_assignment"] = 1
					player:addItem("titanium_staff", 1)
					if player.sex == 0 then 
						player:addItem("earth_mantle", 1)
					end
					
					if player.sex == 1 then
						player:addItem("earth_drapery", 1)
					end
                else 
                    player:dialogSeq({t, "Come back to me when you have those items"}, 0)
                end
	...
	onSayClick = async(function(player, npc)
		...
		if speech == "explore" then
			if player.registry["explore_snake_cave"] == 1 then
				if player:hasItem("fine_snake_meat", 15) == true then
					player:dialogSeq(
						{
							t,
							"Ah, you've returned with proof of your exploration! Let's have a look... Yes, I see the fine snake meat you've brought. Thank you very much for your efforts!",
			...
			if player.registry["explore_lobster_cave"] == 1 then
				if player:hasItem("small_amber_ring", 2) == true then
					player:dialogSeq(
						{
							t,
							"Ah, you've returned with proof of your exploration! Let's have a look... Yes, I see the small amber ring you've brought. Thank you very much for your efforts!",
							"Now, return to the Guru to see your progress and will have a reward waiting for you."
						},
						1
					)
					player:removeItem("small_amber_ring", 2)
					player.registry["explore_lobster_cave"] = 2
					player:msg(
						0,
						"==[Quest Update]==",
						player.ID
		...
		if speech == "frozen staff" and
		player.level >=75 then

			if player.registry["craft_frozen_staff"] == 2 then
				player:dialogSeq({t, "You have craft this staff"}, 0)
				return
			end

			if player.registry["craft_frozen_staff"] == 1 then
				local choice = player:menuString(
				...
				if choice == "Yes" then
					...
					player:hasItem("poet_seal", 25) == true then
						player.karma = player.karma + 1
						player.registry["craft_frozen_staff"] = 2
						player:removeItem("ore",50)
						player:removeItem("metal",50)
						player:removeItem("tarnished_amber",50)
						player:removeItem("poet_seal",25)
						player:addItem(2026, 1)
						...
						local message = "[CRAFT]: " .. player.name .. " successfuly craft frozen staff"
						broadcast(-1, message)

						player:dialogSeq(
							{
								t,
								"Here take the Frozen staff",
							},
							0
						)
					else 
			...
			if player.registry["craft_frozen_staff"] == 0 then
				player:dialogSeq(
					{
						t,
						"Ah, so you have heard tales of a staff that breeze with the fury of frozen.",
						"I can see it in your eyes you are seeking a weapon of true power, we called it Frozer Staff,",
				...
				if choice == "Yes, I am ready" then
					...
					player.registry["craft_frozen_staff"] = 1
					player:msg(
						0,
						"==[Quest Update]==",
						player.ID
