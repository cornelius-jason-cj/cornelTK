WarriorGuruNpc = {
	click = async(function(player, npc)
		...
		if choice == "Take First Assignment" then
			player:dialogSeq(
				{
                    ...
                    "collect 15 pcs items that drop from monster in rat, bat and snake cave",
				},
				1
			)
            choice2 = player:menuString(
			...
			if choice2 == "Yes" then
				if player:hasItem("rat_meat", 15) == true and
				player:hasItem('mica', 15) == true and
				player:hasItem('snake_meat', 15) == true and
				player:hasItem('fine_snake_meat', 15) == true and
				player:hasItem('ginseng_piece', 15) == true and
				player:hasItem('ginseng', 15) == true then
                    player:dialogSeq(
                        {
                            t,
                            "Great job!",
					...
					player:removeItem("snake_meat",15)
					player:removeItem("fine_snake_meat",15)
					...
					if player.sex == 0 then 
						player:addItem("royal_scale_mail", 1)
					end
					
					if player.sex == 1 then
						player:addItem("autumn_mail_dress", 1)
					end
                else 
		...
		if choice == "Take Second Assignment" then
			...
			if choice2 == "Yes" then
				...
				player:hasItem('fox_tail', 5) == true then
					...
					player:addItem("electra", 1)

					if player.sex == 0 then 
						player:addItem("sky_scale_mail", 1)
					end
					
					if player.sex == 1 then
						player:addItem("winter_mail_dress", 1)
					end
                else 
                    player:dialogSeq({t, "Come back to me when you have those items"}, 0)
                end
		...
		if choice == "Take Third Assignment" then
			player:dialogSeq(
				{
                    ...
                    "Collect 25 pcs warrior bone and 1 Large amber ring",
				},
				1
			)
            choice2 = player:menuString(
			...
			if choice2 == "Yes" then
				if player:hasItem("warrior_bone", 25) == true and
				player:hasItem('large_amber_ring', 1) == true then
					...
					player:removeItem("warrior_bone",25)
					player:removeItem("large_amber_ring",1)

                    player.registry["third_assignment"] = 1
					player:addItem("dark_scimitar", 1)
					if player.sex == 0 then 
						player:addItem("earth_scale_mail", 1)
					end
					
					if player.sex == 1 then
						player:addItem("earth_mail_dress", 1)
					end
                else 
                    player:dialogSeq({t, "Come back to me when you have those items"}, 0)
                end
	...
	onSayClick = async(function(player, npc)
		...
		if speech == "explore" then
			if player.registry["explore_bear_cave"] == 1 then
				if player:hasItem("splendid_bear_fur", 1) == true then
					player:dialogSeq(
						{
							t,
							"Ah, you've returned with proof of your exploration! Let's have a look... Yes, I see the splendid bear fur you've brought. Thank you very much for your efforts!",
							"Now, return to the Guru to see your progress and will have a reward waiting for you."
						},
						1
					)
					player:removeItem("splendid_bear_fur", 1)
					player.registry["explore_bear_cave"] = 2
					player:msg(
						0,
						"==[Quest Update]==",
						player.ID
			...
			if player.registry["explore_scorpion_cave"] == 1 then
				if player:hasItem("amber", 15) == true then
					player:dialogSeq(
						{
							t,
							"Ah, you've returned with proof of your exploration! Let's have a look... Yes, I see amber you've brought. Thank you very much for your efforts!",
							"Now, return to the Guru to see your progress and will have a reward waiting for you."
						},
						1
					)
					player:removeItem("amber", 15)
					player.registry["explore_scorpion_cave"] = 2
					player:msg(
						0,
						"==[Quest Update]==",
						player.ID
			...
			if player.registry["explore_ghost_cave"] == 1 then
				if player:hasItem("ash", 5) == true and
				player:hasItem("warrior_bone", 1) == true and
				player:hasItem("rogue_bone", 1) == true and
				player:hasItem("mage_bone", 1) == true and
				player:hasItem("poet_bone", 1) == true then
					player:dialogSeq(
						{
							t,
							"Ah, you've returned with proof of your exploration! Let's have a look... Yes, I see 5 pieces of ash and 1 piece of each type of bone you've brought. Thank you very much for your efforts!",
							"Now, return to the Guru to see your progress and will have a reward waiting for you."
						},
						1
					)
					player:removeItem("ash", 5)
					player:removeItem("warrior_bone", 1)
					player:removeItem("rogue_bone", 1)
					player:removeItem("mage_bone", 1)
					player:removeItem("poet_bone", 1)
					player.registry["explore_ghost_cave"] = 2
					player:msg(
						0,
						"==[Quest Update]==",
						player.ID
		...
		if speech == "frozen sword" and
		player.level >=75 then

			if player.registry["craft_frozen_sword"] == 2 then
				player:dialogSeq({t, "You have craft this sword"}, 0)
				return
			end

			if player.registry["craft_frozen_sword"] == 1 then
				local choice = player:menuString(
				...
				if choice == "Yes" then
					...
					player:hasItem("warrior_seal", 25) == true then
						player.karma = player.karma + 1
						player.registry["craft_frozen_sword"] = 2
						player:removeItem("ore",50)
						player:removeItem("metal",50)
						player:removeItem("tarnished_amber",50)
						player:removeItem("warrior_seal",25)
						player:addItem(2018, 1)
						...
						local message = "[CRAFT]: " .. player.name .. " successfuly craft frozen sword"
						broadcast(-1, message)

						player:dialogSeq(
							{
								t,
								"Here take the Frozen sword",
							},
							0
						)
					else 
			...
			if player.registry["craft_frozen_sword"] == 0 then
				player:dialogSeq(
					{
						t,
						"Ah, so you have heard tales of a sword that breeze with the fury of frozen.",
						"I can see it in your eyes you are seeking a weapon of true power, we called it Frozer Sword,",
				...
				if choice == "Yes, I am ready" then
					...
					player.registry["craft_frozen_sword"] = 1
					player:msg(
						0,
						"==[Quest Update]==",
						player.ID
