function Player.addHealthExtend(player, amount, sleep, deduction, ac, ds)
	local healer
	local ded = 0
	...
	-- else
		-- player.attacker = healer.ID
		-- player:sendHealth(math.floor(healer.damage), 0)
		-- player:addHealth(math.floor(healer.damage), 0)
		-- print(amount)
		-- player:addHealth(amount, 0)
...
function Player.removeHealthExtend(player, amount, sleep, deduction, ac, damageShield)
...
function Player.showBankDeposit(player, npc, bankOwnerName)
	...
	if moneyAmount > 0 then
		...
		npc:talk(0, npc.name .. ": The fee is free")
	end
...
function Player.giveXPStacked(player, amount, bonus)
	...
	if player.state == 1 then
		...
		if (player.level >= 5 and player.class == 0) then
			player:sendMinitext("Its time to meet Guru and choose your class")
			return
		end
		...
		if player.exp + get + bonus >= 4294967295 then
			...
			player:sendMinitext("You cannot gain more experience until you sell some.")
			player:sendStatus()
			return
		end
...
function Player.checkBankCount(player)

	mysql = require "luasql.mysql"
	local env  = mysql.mysql()
	local conn = createDBConn(env)
	--print(env,conn)
	local count = 0
	local cur = assert(conn:execute([[SELECT BnkId FROM Banks WHERE BnkChaId = ]]..stringToNumber(player.ID)..[[ ORDER BY BnkId ASC]])) --pulls all info about this bank item from DB
	count = cur:numrows()
	conn:close()
	env:close()	
