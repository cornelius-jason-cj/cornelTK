heal_poet = {
	cast = function(player, target)
		...
		local spellIdent = "heal_poet"
		local magicCost = player.level + 5
        local spellFX = 4001

		if not player:canCast(1, 1, 0) then
			return
		end
        local multiplier = 0.025
        local damage = (player.maxHealth + player.maxMagic) * multiplier
		local worked = global_heal.cast(player, target, damage, magicCost, spellFX)
	...
	requirements = function(player)
		...
		local description = "Restore health to your target."
